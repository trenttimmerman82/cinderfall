...
(function (CF) {
  ...
  const ORDER = ['carbine', 'shotgun', 'rail', 'pistol'];
  ...
  WP.reset = function (loadout) {
    ...
    this.adsT = 0; this.bloom = 0; this.cycleT = 1; this.fireCd = 0;
  ...
  WP.equip = function (id, instant) {
    ...
    CF.HUD.setWeapon(this.cur.def, this.inv, ORDER);
    this.hudAmmo();
  };
  ...
  WP.give = function (id, silent) {
    ...
    CF.HUD.setWeapon(this.cur.def, this.inv, ORDER, id);
  ...
  WP.updateGrenades = function (dt) {
    for (let i = this.grenadesLive.length - 1; i >= 0; i--) {
      ...
      if (g.fuse <= 0) {
        ...
        CF.Game.explode(g.pos, { radius: 7.5, damage: 180, source: 'player', shake: 1 });
        continue;
      }
      if (!g.rest) {
        g.vel.y -= 18 * dt;
  ...
  WP.update = function (dt, P) {
    ...
    if (live) {
      for (let i = 0; i < 4; i++) if (inp.hit('Digit' + (i + 1))) this.select(ORDER[i]);
    ...
    // ADS
    const canAds = live && inp.mdown[2] && (this.state === 'idle' || this.state === 'reload') && !P.sprinting && !P.mantling;
    ...
    if (live) {
      ...
      if (w.mag === 0 && w.reserve > 0 && this.state === 'idle' && this.fireCd <= -0.25 && !inp.mdown[0]) CF.HUD.hint('Press R to reload');
      else if (w.mag > 0 && w.mag <= Math.ceil(d.mag * 0.25) && this.state === 'idle' && w.reserve > 0) CF.HUD.hint('Reload [R]');
    }

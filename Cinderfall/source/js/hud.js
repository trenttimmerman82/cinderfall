...
(function (CF) {
  ...
  H.killfeed = function (text, extra) {
    ...
    d.innerHTML = text + (extra ? '<em>' + extra + '</em>' : '');
  ...
  H.setWeapon = function (def, inv, order, newId) {
    ...
    order.forEach((id, i) => {
      ...
      html += '<div class="slot' + (id === def.id ? ' active' : '') + (!inv[id] ? ' empty' : '') + (id === newId ? ' new' : '') + '"><kbd>' + (i + 1) + '</kbd>' + d.short + '</div>';
    });
  ...
  H.showScope = function (on, steady, breath) {
    ...
    if (on) {
      ...
      const txt = (steady ? 'STEADY · ' : breath < 4 ? 'HOLD SHIFT TO STEADY · ' : '') + '3.5x · ' + (h ? Math.round(h.t) : '---') + ' m';

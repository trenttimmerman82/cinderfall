...
(function (CF) {
  ...
  const FS_FINAL = [
    ...
    'uniform float uHurt; uniform float uLow; uniform float uSuppress; uniform float uCA; uniform float uSat; uniform vec2 uRes; uniform float uFade;',
    'varying vec2 vUv;',
    ...
    ' col += vec3(-0.012, 0.002, 0.018) * (1.0 - smoothstep(0.0, 0.45, dl)) + vec3(0.02, 0.008, -0.012) * smoothstep(0.5, 1.0, dl);',
    ...
    ' col += (hash(vUv * uRes + fract(uTime * 7.13) * 91.0) - 0.5) * 0.03;',
    ' col *= uFade;',
  ...
  PP.init = function (canvas) {
    ...
    this.mFinal = mat(FS_FINAL, {
      ...
      uHurt: { value: 0 }, uLow: { value: 0 }, uSuppress: { value: 0 }, uCA: { value: 0.0025 }, uSat: { value: 1.05 }, uRes: { value: new THREE.Vector2() }, uFade: { value: 1 }
    });
  ...
  PP.setState = function (s) {
    ...
    if (s.exposure != null) u.uExposure.value = s.exposure;

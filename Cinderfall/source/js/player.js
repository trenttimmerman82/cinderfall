...
(function (CF) {
  ...
  P.update = function (dt) {
    ...
    // crouch / slide
    const crouchHeld = live && inp.down('KeyC');
    const hspeed = Math.hypot(b.vel.x, b.vel.z);
    if (live && inp.hit('KeyC') && b.grounded && (this.sprinting || hspeed > 6.2) && !this.sliding) {
    ...
    // sprint
    const wantSprint = live && inp.down('ShiftLeft') && fwd > 0 && WPN.adsT < 0.3 && !this.sliding && !(WPN.cur && WPN.cur.def.scope && WPN.adsT > 0);
    ...
    // speeds
    const wmul = WPN.cur ? WPN.cur.def.moveMul : 1;
    ...
    if (b.pos.y < -8) { this.damage(999, null, 'The void'); }
    // hazards
    this.hazardT -= dt;
  ...
  P.updateCamera = function (dt, dead) {
    ...
    const bobY = Math.abs(Math.cos(this.bobPhase)) * 0.028 * this.bobAmp * (1 - WPN.adsE * 0.8);
    const bobX = Math.sin(this.bobPhase) * 0.018 * this.bobAmp * (1 - WPN.adsE * 0.8);

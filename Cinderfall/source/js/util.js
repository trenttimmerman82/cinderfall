...
(function (CF) {
  ...
  // ---------------------------------------------------------------- input
  const GAME_KEYS = new Set(['Space', 'Tab', 'KeyW', 'KeyA', 'KeyS', 'KeyD', 'KeyC', 'KeyE', 'KeyG', 'KeyV', 'KeyQ', 'KeyR',
    'ShiftLeft', 'ShiftRight', 'Digit1', 'Digit2', 'Digit3', 'Digit4', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight']);
  const Input = CF.Input = {
    ...
    init(canvas) {
      ...
      window.addEventListener('keydown', (e) => {
        if (this.active && GAME_KEYS.has(e.code)) e.preventDefault();
        if (e.repeat) return;

...
(function (CF) {
  ...
  function lampPost(x, z, dx, dz, o) {
    ...
    let key = 'lampWarm', mat = null;
    if (o.flicker) { key = 'lampFlicker' + (flickerN++); mat = L.mats[key] = L.mats.lampWarm.clone(); }
    L.box(ax - 0.29, 7.05, az - 0.16, ax + 0.29, 7.1, az + 0.16, key, { noCol: true, ao: false });
    return L.lamp(ax, 6.9, az, { color: 0xff9440, intensity: 2.5, distance: 21, cone: 3.8, flicker: o.flicker, mat, poolStrength: 0.36 });
  }
  ...
  Map.build = function () {
    ...
    const sign = new THREE.Mesh(new THREE.PlaneGeometry(16, 2.5), M.sign); sign.position.set(0, 9.3, -7.97); L.scene.add(sign);
